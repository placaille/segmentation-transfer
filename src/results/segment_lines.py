...
def convert_batch_to_frames(source, transformed, segmented):
    frames = []
    for (src, trf, sgt) in zip(source, transformed, segmented):
        stack = torch.stack((src, trf, sgt))
        grid = make_grid(stack, nrow=3, padding=2, normalize=False)
...
@click.argument('transform-model', type=click.Path(exists=True, dir_okay=False, readable=True))
...
def main(input_file, output_file, segmentation_model, transform_model,
    ...
    model_trsf = get_generator_model(transform_model_name, input_channels).to(device)
    model_trsf.load(transform_model)
    ...
    with torch.no_grad():
        ...
        for images in data_iterator:
            ...
            transformed = model_trsf(images.to(device))
            ...
            transformed = torch.mul(transformed.cpu(), 255).to(torch.uint8)
            ...
            gif_frames.extend(convert_batch_to_frames(source, transformed, segmented))
        ...
        del images, source, transformed, segmented,
